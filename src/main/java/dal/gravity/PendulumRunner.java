...
public class PendulumRunner {
	...
	double gEarth = 9.80665; // Earth's gravitational field
	double delta = (args.length == 0) ? .1 : Double.parseDouble (args[0]);
	double sLen = 10, pMass = 10, theta0 = Math.PI/30;
	RegularPendulum rp = new RegularPendulum (sLen, pMass, theta0, new GravityConstant (gEarth), delta);
	SimplePendulum sp = new SimplePendulum (sLen, pMass, theta0, new GravityConstant (gEarth));
	RegularPendulum rpCoarse = new RegularPendulum (sLen, pMass, theta0, new GravityConstant (gEarth), .1);
