...
public class SimplePendulum extends AbstractPendulum {
     ...
     */
    public SimplePendulum (double inLength, double inMass, double inTheta0, GravityModel inGravityModel) {
	super (inLength, inMass, inTheta0, inGravityModel);
	angularFrequency = Math.sqrt (this.getGravityModel ().getGravitationalField () / this.getStringLength ());
	periodOfMotion = 2 * Math.PI 
	    * Math.sqrt (this.getStringLength () / this.getGravityModel ().getGravitationalField ());
    }
